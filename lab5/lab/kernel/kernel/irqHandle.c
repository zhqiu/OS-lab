...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		if (((sf->edx >> 3) % 2 == 0 && destInode.type == DIRECTORY_TYPE) ||   //XXX if O_DIRECTORY is not set
			((sf->edx >> 3) % 2 == 1 && destInode.type == REGULAR_TYPE)) { //XXX if O_DIRECTORY is set
			pcb[current].regs.eax = -1;
			return;
		}
		for (i = 0; i < MAX_DEV_NUM; i ++) {
			if (dev[i].inodeOffset == destInodeOffset && dev[i].state == 1) { // the file refer to a device in use
				pcb[current].regs.eax = i;
				return;
			}
		}
		for (i = 0; i < MAX_FILE_NUM; i ++) {
			if (file[i].inodeOffset == destInodeOffset && file[i].state == 1) { // the file refer to a file in use
				pcb[current].regs.eax = -1;
				return;
			}
		}
		for (i = 0; i < MAX_FILE_NUM; i ++) {
			if (file[i].state == 0) { // the file refer to a file not in use
				file[i].state = 1;
		...
		pcb[current].regs.eax = -1; // no available file[]
		return;
	}
	else { // try to create file
		if ((sf->edx >> 2) % 2 == 0) { //XXX if O_CREATE not set
			pcb[current].regs.eax = -1;
			return;
		}
		if ((sf->edx >> 3) % 2 == 0) { //XXX if O_DIRECTORY not set
...
void syscallLseek(struct StackFrame *sf) {
	...
	switch(sf->ebx) { // whence
		case SEEK_SET:
			...
			break;
		case SEEK_CUR:
			if (offset + file[sf->ecx - MAX_DEV_NUM].offset >= 0 &&
				offset + file[sf->ecx - MAX_DEV_NUM].offset <= inode.size) {
				file[sf->ecx - MAX_DEV_NUM].offset = offset + file[sf->ecx - MAX_DEV_NUM].offset;
				pcb[current].regs.eax = 0;
			}
			else
				pcb[current].regs.eax = -1;
			break;
		case SEEK_END:
			if (offset + inode.size >= 0 &&
				offset + inode.size <= inode.size) {
...
void syscallRemove(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		for (i = 0; i < MAX_DEV_NUM; i ++) {
			if (dev[i].inodeOffset == destInodeOffset) { // the file refer to a device
				pcb[current].regs.eax = -1;
				return;
			}
		}
