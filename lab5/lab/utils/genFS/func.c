...
//	
//	/* check inode & block available or not */
//	if (superBlock->availInodeNum == 0 || superBlock->availBlockNum == 0)
//		return -1;
...
//	(groupDesc + i)->availInodeNum --;
//
//	/* for a directory file, allocate a new block */
//	for (i = 0; i < groupNum; i ++) {
//		if ((groupDesc + i)->availBlockNum >= 1) {
//			blockBitmapOffset = (groupDesc + i)->blockBitmap;
//			blockOffset = (groupDesc + i)->inodeTable;
...
//
//	/* for a directory file, allocate a new block */
//	blockBitmapOffset = groupDesc->blockBitmap;
//	blockOffset = groupDesc->inodeTable;
//	blockBitmap.byte[0] = 0x80;
//	blockOffset += sizeof(Inode) * 8 * superBlock->blockSize / SECTOR_SIZE; // sector as unit
//	groupDesc->availBlockNum --;
//
//	/* write superBlock back
//	 * write groupDesc back */
//	for (i = 0; i < groupNum; i ++) {
...
//	/* write blockBitmap back */
//	fseek(file, blockBitmapOffset * SECTOR_SIZE, SEEK_SET);
//	fwrite((void*)&blockBitmap, sizeof(uint8_t), 1, file); // write the 1st byte of blockBitmap
//
//	/* write inode back */
//	inode.type = DIRECTORY_TYPE;
//	inode.linkCount = 1;
...
int copyData (FILE *file, FILE *fileSrc, SuperBlock *superBlock, GroupDesc *groupDesc, Inode *inode, int inodeOffset) {
	...
	while (size != 0) {
		...
		inode->size += size; //XXX need to write back inode
		i ++;
...
* destFilePath = "/xxx/.../xxxx/" for directory file
* destFilePath = "/xxx/.../xxxx" for directory file
*/
...
* destFilePath = "/xxx//xxx", echo error
* destFilePath = "/", echo error
* destFilePath = "/xxx/.../xxxx/", echo error
* destFilePath = "/xxx/.../xxxx" for regular file
*/
